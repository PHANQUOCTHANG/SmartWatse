...
// Custom Components
import { MapAreaSelect } from "@/features/area/components/MapAreaSelect"; // 🔥 Import Component chọn khu vực

// Types
import { CollectionPointFilterParams } from "../types";

interface CollectionPointFiltersProps {
  filters: CollectionPointFilterParams;
  setFilters: React.Dispatch<React.SetStateAction<CollectionPointFilterParams>>;
  className?: string;
}
...
export const CollectionPointFilters: React.FC<CollectionPointFiltersProps> = ({
  filters,
  setFilters,
  className,
}) => {
  ...
  const handleChange = useCallback(
    (key: keyof CollectionPointFilterParams, value: any) => {
      setFilters((prev) => ({
        ...prev,
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-col lg:flex-row gap-3 justify-between items-start lg:items-center">
          ...
          <div className="relative w-full lg:max-w-md group">
            ...
            <Input
              ...
              placeholder="Tìm kiếm theo tên điểm tập kết..."
        ...
        {showAdvanced && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t border-slate-100 animate-in slide-in-from-top-1 duration-200">
            {/* 1. Filter Area (MapAreaSelect) */}
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold uppercase text-slate-500 tracking-wider ml-1 flex items-center gap-1">
                <MapPin className="size-3" /> Thuộc Khu vực
              </label>

              {/* Component chọn khu vực xịn sò */}
              <MapAreaSelect
                value={filters.areaId || ""}
                onChange={(val) => handleChange("areaId", val)}
                placeholder="Chọn khu vực..."
                // className... (nếu component hỗ trợ)
              />
            </div>

            {/* Có thể thêm các filter khác ở đây nếu cần (vd: Loại rác, Trạng thái...) */}
          </div>
        )}
      </div>
    </div>
